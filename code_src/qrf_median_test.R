...
# mutate variables relevant to qrf model
...
# qrf prediction functions 
predict_real_income_qrf <- function(newdata, qrf_model, mode = "median") {
  ...
  if (mode == "draw") {
    ...
    pred <- predict(qrf_model, newdata = newdata, quantiles = u)$predictions
    return(as.numeric(pred))
  }
...
# function for the income for new entrants into the labor market
...
# Income & Laborforce & Disablility Function
makeIncome_LF_Disability_QRF <- function(YEAR_NOW,
  ...
  # 5. Continuing workers -> Apply QRF
...
write_dataset(samps_all, path = "test1_checkpoint_data/samps_all_qrf3_v4_draw",
...
# year_q is the distribution of income for each year

# === Weight --------

samps_all <- open_dataset("test1_checkpoint_data/samps_all_qrf3_v4_draw") |>
  collect()
